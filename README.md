...
### Q: macOS 提示"无法验证此 App 不包含恶意软件"？

如果双击 `.command` 文件时遇到以下提示：
> "Apple 无法验证 'xxx.command' 不包含恶意软件"

**解决方法：通过终端运行脚本**

1. 按 `Command + 空格` 打开 Spotlight，输入 `终端` 并按回车
2. 复制粘贴以下命令（请根据您的实际路径修改）：

   **创建桌面快捷方式：**
   ```bash
   cd ledger_app && chmod +x 创建桌面快捷方式.command && ./创建桌面快捷方式.command
   ```

   **启动软件：**
   ```bash
   cd ledger_app && chmod +x 启动记账软件.command && ./启动记账软件.command
   ```

3. 按回车执行命令

> 💡 **提示**：`ledger_app` 是示例路径，请替换为您实际存放软件的目录路径。
> 
> 例如，如果您的软件放在"下载"文件夹中，路径则是：`~/Downloads/ledger_app`
